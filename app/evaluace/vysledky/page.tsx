...
function categorizeDimensions(dimensions: Dimension[]): {
  ...
  const total = dimensions.reduce((acc, curr) => acc + curr.normalizedValue, 0);
  ...
  dimensions.forEach((dim) => {
    if (dim.normalizedValue > average) {
      strong.push(dim);
    } else {
      weak.push(dim);
    }
  ...
  // If all dimensions are above average, include the weakest one
  if (strong.length === dimensions.length) {
    weak.push(
      dimensions.reduce((prev, current) =>
        prev.normalizedValue < current.normalizedValue ? prev : current
      )
    );
  }

  // If all dimensions are below average, include the strongest one
  if (weak.length === dimensions.length) {
    strong.push(
      dimensions.reduce((prev, current) =>
        prev.normalizedValue > current.normalizedValue ? prev : current
      )
...
export default function ResultsPage() {
  ...
  const featuredRecommendation = negativeQuestions[0].recommendation;
